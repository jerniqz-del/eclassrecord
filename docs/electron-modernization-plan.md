...
Implemented security-critical slice in `src/main/power-lifecycle.js`: Windows lock/suspend locks the E-Class profile session, creates a restore-point checkpoint, pauses companion LAN discovery, and rejects `/v1/tool-command` until the teacher unlocks the profile again. Resume re-enumerates private IPv4 interfaces, restarts discovery, and asks the renderer to republish the companion snapshot. Automated coverage: `npm run test:electron-profile-auth`, `npm run test:electron-phase2`, and companion command-lock coverage in `scripts/test-companion-sync-service.js` passed 2026-09-07. Wi-Fi/hotspot transitions, duplicate-timer physical gates, and packaged sleep/resume remain. Do not mark Phase 4 Complete until the physical Windows 10/11 + Android matrix is run.
