...
Recorded: 2026-09-05T13:49:39.864Z
...
|---|---:|---:|
| Electron ready | 32.08 ms | <= baseline + max(25%, 250 ms) |
| Probe renderer ready | 906.91 ms | <= baseline + max(25%, 250 ms) |
| Main working set | 83600 KiB | <= baseline + max(20%, 50 MiB) |
| Renderer working set | 73712 KiB | <= baseline + max(20%, 50 MiB) |
| Retained 250-learner sheet heap delta | 67.52 KiB | <= baseline + max(25%, 10 MiB) |
| PDF generation, 250 rows | 155.44 ms | <= baseline + max(30%, 500 ms) |
| Full offline smoke process | 16637.46 ms | <= baseline + max(25%, 2000 ms) |
| Atomic large database save, median | 9.98 ms | <= baseline + max(25%, 50 ms) |
| Backup creation and integrity, median | 0.05 ms | <= baseline + max(25%, 20 ms) |
...
| Excel write, median | 10.1 ms | <= baseline + max(30%, 250 ms) |
| Excel read, median | 7.16 ms | <= baseline + max(30%, 250 ms) |
| 16 MiB update copy and SHA-256, median | 30.63 ms | <= baseline + max(30%, 250 ms) |
| WLAN stop/start reconnect, median | 12.16 ms | <= baseline + max(30%, 250 ms) |
