(function initDashboardOptimizationUi(globalScope) {
  ...
  function optimizeAttention(data) {
    ...
    panel.querySelector('header').innerHTML=`<div><h3>Needs attention <span class="badge">${termAttention.length}</span></h3><small class="workplace-panel__scope">Term ${data.currentTerm}</small></div><div class="tool-segmented"><button type="button" aria-pressed="${prefs.attentionScope!=='all'}" data-eclass-onclick="setDashboardAttentionScope('current')">Current Class</button><button type="button" aria-pressed="${prefs.attentionScope==='all'}" data-eclass-onclick="setDashboardAttentionScope('all')">All Classes</button></div>`;
    panel.querySelector('.workplace-list').innerHTML=visibleAttention.length?visibleAttention.map((item,index)=>`<li class="workplace-attention-item"><button class="workplace-list__item" type="button" data-eclass-onclick="openOptimizedDashboardAttention(${index})"><span class="workplace-list__marker workplace-list__marker--${escText(item.severity)}"></span><span class="workplace-list__content"><strong>${escText(item.title)}</strong><span>${escText(item.detail)}</span></span><span class="workplace-attention-action">${actionLabel(item)}</span></button>${item.dismissible?`<span class="workplace-attention-tools"><button type="button" data-eclass-onclick="snoozeDashboardAttentionItem(${index})">Snooze</button><button type="button" data-eclass-onclick="dismissDashboardAttentionItem(${index})">Dismiss</button></span>`:''}</li>`).join(''):`<li class="workplace-empty">You are caught up for Term ${data.currentTerm}. No grading work needs attention.</li>`;
  }
