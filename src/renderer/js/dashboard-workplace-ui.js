  ...
  function renderWorkplace() {
    ...
    target.innerHTML = `
      <section class="workplace-hero" aria-labelledby="workplaceGreeting">
        ...
        <div class="workplace-context"><div class="workplace-context__selectors"><label><span class="field-label">Working class</span><select id="dashboardWorkplaceClass" class="field-select" data-eclass-onchange="selectDashboardWorkplaceAssignment(this.value)" ${options ? '' : 'disabled'}>${options || '<option>No teaching loads yet</option>'}</select></label><label><span class="field-label">Term</span><select id="dashboardWorkplaceTerm" class="field-select" aria-label="Dashboard term" data-eclass-onchange="setDashboardAnalyticsTerm(this.value)" ${selectedId ? '' : 'disabled'}>${['1','2','3'].map(term => `<option value="${term}" ${term === snapshot.currentTerm ? 'selected' : ''}>Term ${term}</option>`).join('')}</select></label></div><button class="btn btn-primary workplace-continue" type="button" data-eclass-onclick="openDashboardWorkplaceAction('grading', '${esc(selectedId)}', '${esc(snapshot.currentTerm)}')" ${selectedId ? '' : 'disabled'}>Continue grading</button></div>
      </section>
      ...
      <div class="workplace-grid">
        ...
        <section class="workplace-panel" aria-labelledby="workplaceUpcomingTitle"><header class="workplace-panel__header"><h3 id="workplaceUpcomingTitle">Today &amp; upcoming</h3><button class="btn btn-ghost btn-sm" type="button" data-eclass-onclick="openCalendarView()">View calendar</button></header><div class="workplace-panel__body"><ul class="workplace-list">${upcoming}</ul></div></section>
        <section class="workplace-panel" aria-labelledby="workplaceTasksTitle"><header class="workplace-panel__header"><h3 id="workplaceTasksTitle">My tasks</h3><button class="btn btn-ghost btn-sm" type="button" data-eclass-onclick="showDashboardWorkplaceTaskModal()">Add task</button></header><div class="workplace-panel__body"><ul class="workplace-list">${tasks}</ul></div></section>
      </div>
      </div>`;
  }
