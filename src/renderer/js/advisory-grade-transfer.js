...
(function initAdvisoryGradeTransfer(globalScope) {
  ...
  function renderWorkspacePanel(workspace, advisoryClass) {
    ...
    const rosterRows = rosterLearners.length ? rosterLearners.map((learner, index) => `<tr><td>${index + 1}</td><td class="advisory-roster__lrn">${globalScope.esc(learner.lrn || '—')}</td><td><strong>${globalScope.esc(globalScope.AdvisoryRoster.displayName(learner))}</strong></td><td>${globalScope.esc(learner.sex || '—')}</td><td>${globalScope.esc(globalScope.formatLearnerBirthdate?.(learner.birthdate) || learner.birthdate || '—')}</td><td>${globalScope.esc(learner.enrollmentStatus || 'active')}</td><td>${globalScope.esc(learner.source || 'manual')}</td><td><div class="advisory-roster-row-actions"><button class="btn btn-ghost btn-sm" type="button" data-edit-advisory-learner="${globalScope.esc(learner.id)}">Edit</button><button class="btn btn-danger btn-sm" type="button" data-remove-advisory-learner="${globalScope.esc(learner.id)}">Remove</button></div></td></tr>`).join('') : '<tr><td colspan="8"><div class="advisory-roster__empty">No learners yet. Use the roster actions above to import or add learners.</div></td></tr>';
    ...
    panel.innerHTML = `
      ...
      <section id="advisoryRosterPanel" role="tabpanel" data-advisory-panel="roster" hidden>
        ...
        <div class="advisory-roster-table-wrap advisory-page-roster-table"><table class="advisory-roster-table"><thead><tr><th>#</th><th>LRN</th><th>Official Name</th><th>Sex</th><th>Birthdate</th><th>Status</th><th>Source</th><th>Actions</th></tr></thead><tbody>${rosterRows}</tbody></table></div>
      </section>
