...
if (!nextVersion) {
  ...
  try {
    ...
    // If no tags exist, default to patch
  }

  if (latestTag) {
    try {
      const commitLogs = execSync(`git log ${latestTag}..HEAD --oneline`, { encoding: 'utf8' }).trim();
      if (commitLogs) {
        ...
        for (const line of lines) {
          ...
          if (message.includes('BREAKING CHANGE') || /^[a-zA-Z0-9_-]+(\([a-zA-Z0-9_-]+\))?\!\:/.test(message)) {
            hasBreaking = true;
            break;
          }
          if (/^feat(\([a-zA-Z0-9_-]+\))?\:|^feature(\([a-zA-Z0-9_-]+\))?\:/.test(message.toLowerCase())) {
            hasFeature = true;
          }
        }
        if (hasBreaking) {
      ...
      console.warn('Could not determine commits since last tag. Defaulting to patch bump.');
    }
  }
  ...
  if (parts.length === 3) {
    ...
    console.log(`Auto-detected SemVer bump: ${bumpType.toUpperCase()}`);
  } else {
...
console.log('Building local release assets...');
runCmd('npm run build');

console.log('Uploading release assets to GitHub...');
runCmd(`gh release upload v${nextVersion} "dist/E-Class Record Setup ${nextVersion}.exe" "dist/E-Class Record Setup ${nextVersion}.exe.blockmap" --clobber`);
