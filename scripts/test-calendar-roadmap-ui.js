...
assert(ui.includes('isMockCalendarEvent'));
assert(ui.includes('removeMockEvents'));
assert(ui.includes('legacySeedIds'));
assert(ui.includes("'deped-q1-exam-1'"));
assert(ui.includes('globalScope.saveDatabase?.()'));
assert(!ui.includes("typeof baseSeed==='function'"));
const calendarCss=fs.readFileSync(path.join(root,'css/calendar.css'),'utf8');
assert(calendarCss.includes('grid-auto-rows: 84px'));
assert(calendarCss.includes('height: 84px'));
